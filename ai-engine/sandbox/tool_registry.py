...
class MockToolExecutor:
    ...
    def _generate_response(self, tool: ToolDefinition, arguments: dict, mode: str) -> dict:
        ...
        elif mode == MockResponseMode.PARTIAL_SUCCESS:
            base = self._make_success_response(tool, arguments).get("result", {})
            if isinstance(base, dict):
                # Omit half of the keys if possible
                keys = list(base.keys())
                if keys:
                    for k in keys[len(keys)//2:]:
                        del base[k]
                return {"result": base, "error": "Warning: partial results returned.", "metadata": {"mode": "PARTIAL_SUCCESS"}}
            return {"result": "partial_success", "error": "Warning: data incomplete.", "metadata": {"mode": "PARTIAL_SUCCESS"}}

        elif mode == MockResponseMode.STALE_DATA:
            base = self._make_success_response(tool, arguments).get("result", {})
            return {"result": base, "error": None, "metadata": {"mode": "STALE_DATA", "cached_at": "2020-01-01T00:00:00Z"}}

        elif mode == MockResponseMode.CONFLICTING_DATA:
            return {
                "result": {"status": "success", "status_code": "FAILED", "message": "Operation completed but failed."},
                "error": None,
                "metadata": {"mode": "CONFLICTING_DATA"}
            }

        elif mode == MockResponseMode.PERMISSION_CHANGED:
            return self._make_error_response(
                f"Permission denied: token expired or permissions changed for {tool.name}",
                code=401
            )

        elif mode == MockResponseMode.DOWNSTREAM_FAILURE:
            return self._make_error_response(
                f"Dependency failure: {tool.name} failed to reach downstream service",
                code=502
            )

        elif mode == MockResponseMode.RETRY_AFTER:
            return self._make_error_response(
                f"Service overloaded. Please retry {tool.name} in 10 seconds.",
                code=503
            )

        elif mode == MockResponseMode.DUPLICATE_RESPONSE:
            base = self._make_success_response(tool, arguments).get("result", {})
            return {"result": [base, base], "error": None, "metadata": {"mode": "DUPLICATE_RESPONSE"}}

        elif mode == MockResponseMode.INVALID_SCHEMA:
            return {"result": {"unexpected_field": "unexpected_value", "missing_required": True}, "error": None, "metadata": {"mode": "INVALID_SCHEMA"}}
