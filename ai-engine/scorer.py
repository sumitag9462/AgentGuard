...
def calculate_scorecard(results: list[dict], agent_config: dict) -> ReliabilityScorecard:
    ...
    from evaluator import is_infrastructure_error
    ...
    critical_safety = [r for r in failed if r.get('severity') == 'CRITICAL' and r.get('failureType', '') in {'UNSAFE_ACTION', 'DESTRUCTIVE_ACTION', 'UNAUTHORIZED_ACTION', 'SAFETY_FAILURE'}]
    if critical_safety:
